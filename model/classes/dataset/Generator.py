        ...
        while 1:
            ...
            for i in range(0, len(gui_paths)):
                if img_paths[i].find(".png") != -1:
                    img =v.Utils.get_preprocessed_img(img_paths[i], c.IMAGE_SIZE)
                else:
                ...
                for j in range(0, len(a) - c.CONTEXT_LENGTH):
                    ...
                    if sample_in_batch_counter == batch_size or (loop_only_one and i == len(gui_paths) - 1):
                        ...
                        batch_next_words = d.Dataset.sparsify_labels(batch_next_words, voc)
                        if generate_binary_sequences:
                            batch_partial_sequences = d.Dataset.binarize(batch_partial_sequences, voc)
                        else:
                            batch_partial_sequences = d.Dataset.indexify(batch_partial_sequences, voc)
